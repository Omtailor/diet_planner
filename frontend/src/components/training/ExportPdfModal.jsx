...
/**
 * Bottom-sheet modal for selecting a date range and exporting
 * the training plan as a PDF.
 */
export default function ExportPdfModal({
  ...
  const isInvalid = exportStartDate > exportEndDate;
  const daysSelected = exportStartDate && exportEndDate
    ? Math.max(0, Math.round(
        (new Date(exportEndDate) - new Date(exportStartDate)) / (1000 * 60 * 60 * 24) + 1
      ))
    : 0;
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div>
            ...
            <input
              ...
              style={{
                ...
                borderRadius: 16, fontFamily: FONT, fontSize: '1rem',
                fontWeight: 600, color: 'var(--color-text)', outline: 'none',
              }}
            />
          </div>
          <div>
        ...
        {exportStartDate && exportEndDate && (
          <div style={{ textAlign: 'center', marginBottom: 20 }}>
            <span style={{
              ...
              background: 'rgba(52,199,89,0.12)', color: 'var(--color-accent)',
        ...
        <button
          onClick={onExport}
          disabled={!exportStartDate || !exportEndDate || isInvalid}
          style={{
            width: '100%', padding: '16px',
            background: isInvalid ? 'rgba(0,0,0,0.1)' : 'var(--color-accent)',
            border: 'none', borderRadius: 16,
            color: isInvalid ? 'var(--color-text-muted)' : '#ffffff',
            fontFamily: FONT, fontWeight: 800, fontSize: '1rem',
            cursor: isInvalid ? 'not-allowed' : 'pointer',
            boxShadow: isInvalid ? 'none' : '0 8px 24px rgba(52,199,89,0.3)',
