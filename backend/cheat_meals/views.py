    ...
    def post(self, request):
        ...
        try:
            ...
            for img_file in images:
                img_obj = CheatMealImage.objects.create(
                    cheat_meal=cheat_meal, image=img_file
                )
            ...
            cheat_meal.quantity_description = ai_data.get(
                "portion_description", ""
            )  # ← fixed
            cheat_meal.ai_estimated_calories = int(ai_data.get("estimated_calories", 0))
            cheat_meal.ai_confidence = float(
                ai_data.get("confidence_level", 0.5)
            )  # ← fixed
            cheat_meal.ai_raw_response = ai_data  # ← fixed

            if request.data.get("user_edited_calories"):
                cheat_meal.user_edited_calories = int(
                    request.data["user_edited_calories"]
                )
            ...
            if cheat_meal and cheat_meal.pk:
                cheat_meal.delete()
            return Response(
                {"error": "AI analysis failed. Please try again."}, status=500
            )
...
class ManualCheatMealFollowUpView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ai_data = analyze_food_text(
                cheat_meal.food_name or "", follow_up_answer=answer
            )  # ← use food_name (no manual_description field)
        except Exception as e:
            return Response(
                {"error": "AI analysis failed. Please try again."}, status=500
            )
        ...
        cheat_meal.ai_estimated_calories = int(ai_data.get("estimated_calories", 0))
        cheat_meal.ai_confidence = float(
            ai_data.get("confidence_level", 0.4)
        )  # ← fixed
        cheat_meal.ai_raw_response = ai_data  # ← fixed
        cheat_meal.save()
        apply_cheat_meal_adjustment(request.user, cheat_meal)
...
class EditCheatMealCaloriesView(APIView):
    ...
    def patch(self, request, pk):
        ...
        if cal is None or not isinstance(cal, (int, float)) or cal < 0:
            return Response(
                {"error": "user_edited_calories must be a non-negative number."},
                status=400,
            )
...
class CheatMealHistoryView(APIView):
    ...
    def get(self, request):
        meals = CheatMeal.objects.filter(user=request.user).order_by(
            "-logged_at"
        )  # ← fixed
