...
class DayMeal(models.Model):
    ...
    weekly_plan = models.ForeignKey(WeeklyPlan, on_delete=models.CASCADE, related_name='day_meals')
...
class MealSlot(models.Model):
    ...
    day_meal = models.ForeignKey(DayMeal, on_delete=models.CASCADE, related_name='meal_slots')
    ...
    def save(self, *args, **kwargs):
        if self.food_item and self.quantity_g:
            ratio = self.quantity_g / self.food_item.serving_size_g
            self.calories = round(self.food_item.calories * ratio, 1)
            self.protein_g = round(self.food_item.protein_g * ratio, 1)
            self.carbs_g = round(self.food_item.carbs_g * ratio, 1)
            self.fats_g = round(self.food_item.fats_g * ratio, 1)
